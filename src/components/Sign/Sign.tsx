...
import { API_URL, client } from '../../api/fetchClient';
...
export const Sign = () => {
  ...
  const loadData = async () => {
    try {
      ...
      } else if (userData.body.authorized === false) {
  ...
  const postData = async (data: any) => {
      try {
        await axios.post<any>(API_URL + '/user', data, {
        ...
        <form onSubmit={handleSubmit} className='d-flex flex-column justify-content-center align-items-center gap-2'>
          ...
          <div className='sign__input-container'>
            <input
              ...
              className='sign__input'
            />
            <span className={classNames('sign__input--eye', { active: InputPasswordType === 'text' })}
