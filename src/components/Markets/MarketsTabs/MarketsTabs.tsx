...
export const MarketsTabs: FC<Props> = ({ currentTab, tabChange: handleCurrentTabChange }) => {
  return (
    ...
    <div onClick={() => handleCurrentTabChange(MarketsTabsType.alerts)} style={{cursor: 'pointer'}} className={currentTab === MarketsTabsType.alerts ? 'text-white' : ''}>Alerts<sup style={{ color: '#ff363a', fontSize: '1.5rem', fontWeight: '700' }}>.</sup></div>
