...
export const MarketsTableSubRowAdditional: FC<Props> = ({ data, currentSymbol, counterEarning, marketPrice = 0, isFilled = false }) => {
  ...
  const profitCounterQuantityCalculate = isTakeProfit
    ? new bigDecimal(data.sellFilledTakeProfitQuantity).multiply(marketPriceDecimal).subtract(new bigDecimal(data.sellFilledTakeProfitCounterQuantity)).round(data.counterRound, bigDecimal.RoundingModes.FLOOR)
    : new bigDecimal(data.sellFilledStopLossQuantity).multiply(marketPriceDecimal).subtract(new bigDecimal(data.sellFilledStopLossCounterQuantity)).round(data.counterRound, bigDecimal.RoundingModes.FLOOR);
